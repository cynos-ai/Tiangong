...
const CAPABILITIES = Object.freeze({ leader: "coordination-control", architect: "project-read-only", challenger: "project-read-only", developer: "local-development", reviewer: "project-read-only", tester: "controlled-testing" });
...
const TOOL_GROUPS = Object.freeze({ leader: ["coordination", "skill-runtime"], architect: ["skill-runtime"], challenger: ["skill-runtime"], developer: ["skill-runtime", "native-runner"], reviewer: ["skill-runtime"], tester: ["skill-runtime", "controlled-test"] });
const MANIFEST_FIELDS = new Set(["kind", "schemaVersion", "packageId", "version", "responsibility", "displayName", "instructions", "runtime", "model", "capabilityProfile", "toolGroups", "sessionPolicy", "installedSkills"]);
...
export async function loadAgentPackage(packageRootInput) {
  ...
  if (manifest.kind !== "tiangong.agent-package" || manifest.schemaVersion !== 1 || !ID.test(manifest.packageId ?? "") || !VERSION.test(manifest.version ?? "") || !RESPONSIBILITIES.has(manifest.responsibility) || manifest.responsibility !== directoryName) fail("AGENT_PACKAGE_INVALID", `${directoryName}: identity is invalid`);
  if (manifest.instructions !== "instructions.md" || !["openclaw-built-in", "codex-app-server"].includes(manifest.runtime) || typeof manifest.model !== "string" || manifest.model.length === 0) fail("AGENT_PACKAGE_INVALID", `${directoryName}: runtime projection is invalid`);
  if (manifest.capabilityProfile !== CAPABILITIES[manifest.responsibility] || manifest.sessionPolicy !== SESSION_POLICIES[manifest.responsibility]) fail("AGENT_PACKAGE_INVALID", `${directoryName}: capability or session policy is invalid`);
  const instructions = await readFile(resolve(packageRoot, manifest.instructions), "utf8"); bounded(manifest.displayName, "displayName", 64); if (instructions.length === 0 || Buffer.byteLength(instructions) > 16 * 1024) fail("AGENT_PACKAGE_INVALID", `${directoryName}: instructions are invalid`);
...
export async function resolveMemberAgent({ memberConfig, packageRoot, skillsRoot } = {}) {
  ...
  if (agentPackage.responsibility !== responsibility || agentPackage.runtime !== memberConfig.runtime || agentPackage.model !== memberConfig.model || agentPackage.capabilityProfile !== memberConfig.capabilityProfile) fail("MEMBER_AGENT_MISMATCH", "MemberConfig does not match its Agent package");
  ...
  return freeze({ memberId: memberConfig.memberId, memberRevision: memberConfig.revision, agentPackage, effectiveSkills: skills, capabilityProfile: agentPackage.capabilityProfile, sessionPolicy: agentPackage.sessionPolicy });
}
...
export async function resolveAgentRuntimeFromEnvironment(env = process.env, options = {}) {
  const responsibility = normalizeResponsibility(env.TIANGONG_MEMBER_RESPONSIBILITY); const packageId = env.TIANGONG_MEMBER_AGENT_PACKAGE_ID; const packageVersion = env.TIANGONG_MEMBER_AGENT_PACKAGE_VERSION; const capabilityProfile = env.TIANGONG_MEMBER_CAPABILITY_PROFILE;
  if (!RESPONSIBILITIES.has(responsibility) || !ID.test(packageId ?? "") || !VERSION.test(packageVersion ?? "")) fail("MEMBER_AGENT_ENV_INVALID", "Agent package deployment projection is missing or invalid");
  ...
  if (!agentPackage || agentPackage.version !== packageVersion || agentPackage.responsibility !== responsibility || agentPackage.runtime !== env.TIANGONG_MEMBER_RUNTIME || agentPackage.model !== env.TIANGONG_MEMBER_MODEL || agentPackage.capabilityProfile !== capabilityProfile) fail("MEMBER_AGENT_MISMATCH", "Environment does not match the installed Agent package");
  ...
  return freeze({ agentPackage, effectiveSkills: skills, capabilityProfile, memberId: env.TIANGONG_MEMBER_ID ?? env.AGENTTEAMS_WORKER_NAME ?? null });
}

export { CAPABILITIES, SESSION_POLICIES };
