...
function fixture() {
  ...
  const work = createWorkSpec({ workId: "work-native-binding", revision: 1, goal: "Implement the bounded change", scope: ["fixture"], doneWhen: ["one result"], createdAt: NOW });
  ...
  const task = createTaskSpec({ taskId: "task-native-binding", workId: work.workId, assigneeMemberId: member.memberId, objective: "Run the bounded implementation", constraints: ["one bounded result"], inputs: [], createdAt: NOW });
  ...
  const taskBinding = createTaskBinding({ taskId: task.taskId, projectId: projectBinding.projectId, playbookStepId: "software-change-delivery-transition-v1", taskKind: "implement", revisionIndex: 0, assignee: member.workerName, objective: task.objective, completionContractDigest: sha256(task.constraints.join("\n")), sourceProfileDigest: sha256("profile-native-binding"), sourceSkillId: "implementor-controlled-implementation-v1", sourceSkillDigest: sha256("skill-native-binding"), inputRefs: [], createdAt: NOW });
