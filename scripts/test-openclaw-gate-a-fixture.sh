...
grep -Fq 'ARG TIANGONG_CODEX_TRANSPORT=native-responses' "${DOCKERFILE}" || fail 'DeepSeek canary must use the native Responses route.'
grep -Fq 'ARG TIANGONG_CODEX_BRIDGE=none' "${DOCKERFILE}" || fail 'DeepSeek canary must not select a Chat bridge.'
