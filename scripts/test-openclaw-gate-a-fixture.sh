...
grep -Fq 'ARG TIANGONG_CODEX_CAPABILITY_CACHE_SHARED=0' "${DOCKERFILE}" || fail 'Canary image must fail closed until the deployment confirms a shared capability cache.'
