...
- Added six versioned long-lived Agent packages for Leader, Architect, Challenger, Developer, Reviewer, and Tester, with fixed runtime/model/tool-group and one-Work/one-Task logical-session policy.
...
- Removed the full file-backed CoordinationStore and extracted only its pure record constructors and validators.
- Removed inactive Codex/OpenCodex, native Runner, deployment/recovery, pending Operation, Approval command, operation-digest, fixed-role/Playbook, and hash-chain Evidence implementations, image targets, scripts, smoke scenarios, and tests without compatibility shims.
- Removed obsolete Operation/Approval placeholders from the current local-delivery CloseGuard; real external-write facts return with their Adapter in M9.
