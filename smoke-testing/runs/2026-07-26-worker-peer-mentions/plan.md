...
Issue #11 / Draft PR #12 provide the deterministic target, one-reply correlation, and pre-turn Harness ingress marker without modifying AgentTeams. Issue #13 / Draft PR #14 and the owned receiver provide the lower-layer phase trace. Direct plugin/runtime and long-lived official Gateway probes now pass both error/retry and success paths, so the remaining unclassified actual-request path is specific to Matrix-dispatched host context, concurrent channel/session lifecycle, or request-delivery timing in the released Worker. Repeated model retries, longer observation windows, or prompt tuning must not be used to manufacture reliability. The next work is source-backed comparison of the Matrix-dispatched Harness parameters and host lifecycle against the passing no-model Gateway probes, not another Matrix/model run. Phase 0B remains red until transport and observability pass together.
