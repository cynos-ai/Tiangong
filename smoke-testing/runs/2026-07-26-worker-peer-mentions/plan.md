...
### Deterministic transport correction — READY FOR FOCUSED INTEGRATION
...
**BLOCKED pending focused integration.** Attempts 12–20 prove Admin → Coordinator account visibility and official OpenClaw-to-Tiangong Harness dispatch. Attempt 16 passed the complete bounded four-event transport, Harness, terminal-quiescence, and Leader-silence oracle, but its required event-correlated trace was empty because of the now-fixed receiver identity defect. Attempt 19 proved the hardened receiver accepts real Worker exports and correlates the current event, but only established an active pi turn under the old overbroad `model.start` label. Attempt 20 used the refined oracle and proved real response receipt, stream progress, and complete model/Harness operations, while the resulting Coordinator event still omitted the required ping marker and never woke Engineer. No run has yet passed transport and observability in the same attempt.

Issue #11 / Draft PR #12 provide the deterministic target, one-reply correlation, and pre-turn Harness ingress marker without modifying AgentTeams. Issue #13 / Draft PR #14 and the owned receiver now provide a working lower-layer phase trace. The latest observed blocker was a completed Coordinator model turn whose emitted event did not satisfy the deterministic ping-marker contract. The source-backed correction removes model wording from that transport boundary. Attempt 21 proved the deterministic Matrix/Harness/trace chain but exposed and fixed deferred persistence for control-only sessions; no post-fix run has yet proven every completion, session, Leader, and cleanup oracle together. Repeated model retries, longer observation windows, timeout mutation, or prompt tuning must not be used to manufacture reliability. Phase 0B remains red until one bounded run passes the complete transport and observability oracle together.
