...
**BLOCKED.** Attempts 12–14 prove Admin → Coordinator account visibility and official OpenClaw-to-Tiangong Harness dispatch, but the Coordinator turn did not complete within the unchanged bounded window. Attempt 13 exposed a fixed receiver root-shape defect; Attempt 14 showed that event-relative trace selection alone was insufficient to classify no-match. Earlier complete loops remain insufficient because they predate bounded terminal correlation or failed terminal quiescence.

Issue #11 / Draft PR #12 provide the deterministic target, one-reply correlation, and pre-turn Harness ingress marker without modifying AgentTeams. Issue #13 / Draft PR #14 and the corrected owned receiver oracle provide the lower-layer phase trace required before another attempt. Repeated model retries, longer observation windows, or prompt tuning must not be used to manufacture reliability. The receiver root shape, counters, unmatched inventory, and actual plugin export path are now covered by deterministic and same-network no-model probes. One final unchanged diagnostic run is justified only after commit `c051a5f` passes CI; its purpose is to classify the in-flight boundary, not to retry the model into success. Phase 0B remains red until the complete strengthened oracle passes.
