...
Only the final command creates the disposable Team and performs real Harness turns. The deterministic transport controls do not dispatch a model request.
...
### Attempt 19 — BLOCKED (receiver fixed; Coordinator pi turn remained in flight)
...
- The event-correlated trace reached `harness.start`, runtime setup, Gateway resolution, session readiness, `pi.agent_turn.start`, and the then-named `model.start`. The Harness marker independently remained `status=running`. Subsequent pinned-source review showed that `model.start` was emitted from pi `turn_start`, before context conversion, credential resolution, and the provider stream call; the open span therefore did not prove that an external model request had been dispatched.
...
- This run is an in-flight pi-turn boundary after session readiness, not a receiver, correlation, account-visibility, pre-Harness, or channel-health ambiguity. It does not distinguish provider preparation, waiting for response headers, or response streaming. The transport observer has 36 incremental Matrix sync windows with a 10-second server timeout, while released AgentTeams supplies the independently enforced 1800-second Harness timeout. `status=running` at observer expiry therefore proves failure to complete within the smoke window, not a Harness-timeout defect or proof that the call would never terminate. Contract tests pin both observer values against silent mutation.
...
### Post-Attempt-19 model activity refinement

- Pinned pi source establishes that `turn_start` precedes context conversion, credential resolution, and `streamFunction`; the old `model.start` name overclaimed the observed boundary.
...
- A paired silent local provider fixture accepted the request but never returned headers. The exact official Gateway path ended at the fixture-only five-second Harness timeout with two accepted export requests, 13 spans, zero rejections, `pi.turn.start` and `model.request.ready`, no response/progress phases, and sanitized `timeout` outcomes on both `gen_ai.chat` and the Harness root. Together, the two probes deterministically distinguish genuine stream progress from waiting on a silent provider; before an explicit timeout, silence remains only a suspected stall rather than proof of deadlock.
- The failure oracle now projects correlated spans into fixed boolean peer-transport/pi-turn/request/response/progress/retry/model-terminal activity facts. It reports observations rather than inferring event order or declaring deadlock, and deterministic fixtures cover both streaming completion and silent-provider timeout without retaining unallowlisted input fields.
- No external model or Matrix turn was used for this state-machine change.

### Attempt 20 — BLOCKED (model progressed and completed; required ping was absent)

- The new source-backed activity discriminator, both no-model protocol probes, all stacked deterministic/image checks, and required CI passed before authorizing one unchanged bounded run. Prompt, model, released 1800-second timeout, observer, AgentTeams release, and transport oracle were unchanged.
- Coordinator account visibility and Harness passed. One Coordinator message appeared and its persistent session contained the nonce, but the event did not contain the required `TG_PEER_PING` marker. Engineer therefore had no visible ping, Harness turn, pong, or session; no terminal event formed. Both Matrix channels remained stable and stock Leader emitted zero messages.
- The exact event-correlated trace completed through `pi.turn.start`, `model.request.ready`, `model.response.received`, `model.response.start`, and `model.response.progress`; `gen_ai.chat`, pi agent turn, and Harness root all ended `complete`. The fixed activity facts were all true through response progress and model completion, with no retry, timeout, or abort.
- The hardened receiver accepted seven export requests containing 41 spans with zero rejections. This run proves the model was neither silent nor stuck: it received a response, streamed real progress, and completed inside the observer. The failure is that the completed model output did not satisfy the deterministic transport marker contract. Model prose remains private and is not used as machine Evidence.
- Repeating the same model call or tuning the prompt to seek a favorable sample is not authorized. Phase 0B remains red because no Engineer wake-up or return transport occurred in this attempt.
- Exact Team/member/container/alias/storage/receiver/trace cleanup passed.

### Deterministic transport correction

- Attempt 20 classified the remaining failure as an overfitted scenario/product boundary: exact transport markers were delegated to stochastic model prose even though the smoke intended to prove Matrix/Harness transport.
- Issue #15 / Draft PR #16 adds a bounded Tiangong-owned transport diagnostic. The OpenClaw adapter derives outbound authority only from authenticated effective `groupAllowFrom - dm.allowFrom`; the ingress sender must be allowed, start requires exactly one outbound peer, and pong must match the authenticated expected peer plus pending nonce.
- `TG_PEER_START/PING/PONG` are handled inside real Tiangong Harness turns. Runtime code owns exact ping/pong/terminal text, official OpenClaw remains the Matrix sender, and the test driver still sends only the initial Admin event. Ordinary prompts continue through the model path.
- Malformed/ambiguous controls, unknown senders, duplicate/unbounded peers, replayed turn IDs, nonce mismatch, and consumed plans fail closed. Deterministic positive, negative, replay, adjacent normal, runtime-no-provider-dispatch, adapter authorization, session, and observability tests pass.
- Completed controls emit `peer.transport.start/ping/pong`; model spans are intentionally absent. The completion oracle remains the four ordered Matrix events, three completed correlated Harness roots, two Harness/session changes, terminal quiescence, Leader silence, and exact cleanup.
- The start event is reduced to the reserved control plus nonce. It contains no Engineer identity fragments, Leader identity, or model instructions.

### Attempt 21 — BLOCKED (transport and traces passed; deterministic session output was not flushed)

- The first deterministic focused run passed the exact four-event Admin start → Coordinator ping → Engineer pong → Coordinator terminal chain, exact code-owned bodies and `m.mentions`, both Harness markers, all three event-correlated completed Harness roots, `peer.transport.start/ping/pong`, zero model spans for those controls, Matrix readiness, and zero Leader messages.
- It then failed the existing persistent-session nonce oracle for Coordinator. Exact cleanup passed.
- Source inspection identified the boundary: pi `SessionManager` deliberately defers a new session file until an assistant message exists. The transport handler had appended only a custom control message, so a control-only session remained in memory even though Matrix delivery and tracing completed.
- PR #16 commit `aeb089a` records deterministic transport output as a machine-authored assistant session entry with zero usage and explicit `tiangong-control` / `peer-transport-v1` metadata. It does not claim a model call. A lower-layer regression now proves the JSONL exists and contains the nonce while the unreachable provider receives no dispatch.
- A post-fix focused run remains required; the transport success from this failed attempt does not waive the session or cleanup oracle.

### Attempt 22 — PASS (deterministic peer transport and complete correlated observability)

- The unchanged post-fix focused run passed the exact four-event Admin start → Coordinator ping → Engineer pong → Coordinator terminal chain. All bodies were the exact code-owned protocol forms, ping/pong carried only the required structured peer mention, terminal carried no peer/Leader mention, and the bounded grace window observed no later Worker/Leader event.
- Both Tiangong Harness markers changed after baseline and ended `pass`; Coordinator and Engineer persistent sessions both contained the nonce.
- Event-correlated Harness roots completed for Admin start, Coordinator ping, and Engineer pong with the exact `peer.transport.start`, `peer.transport.ping`, and `peer.transport.pong` phases. The strengthened oracle also proved no `gen_ai.chat` or `tiangong.pi.agent_turn` span existed for any deterministic control turn.
- Both official Matrix channels remained running, connected, healthy, non-restarting, and stable across the readiness window. Stock Leader emitted zero messages and its model/session snapshot remained unchanged.
- Team/member/container/alias/storage/helper/receiver/trace cleanup completed exactly, with independent post-run checks confirming the reserved Team, containers, and trace path were absent.
- This is the first single run to pass the complete transport, Harness, session, event-correlated observability, terminal quiescence, Leader silence, and cleanup oracle together.

### Current result

**PASS.** Attempts 12–20 isolated Matrix visibility, Harness ingress, reply targeting, timeout, receiver identity, and model-activity ambiguities without weakening the oracle. Attempt 20 finally proved that stochastic model wording—not liveness—was the remaining transport-marker defect. Issue #15 / Draft PR #16 moved that exact boundary into authenticated, bounded Worker code. Attempt 21 proved the new transport/trace chain and exposed deferred control-only session persistence; commit `aeb089a` fixed it. Attempt 22 then passed every transport, Harness, session, event-correlated observability, terminal-quiescence, Leader-silence, and exact-cleanup oracle together without dispatching a model request for transport controls.

Issue #11 / Draft PR #12 provide authenticated target projection, one-reply correlation, and exact Harness timeout. Issue #13 / Draft PR #14 provide sanitized lower-layer tracing. Issue #15 / Draft PR #16 provide the deterministic transport diagnostic. Phase 0B is green without modifying AgentTeams or released timeout/observer values. This proves transport only; it does not establish trusted Role Profiles, formal Assignment/Result, Ledger, recovery, independent verification, or Team assurance.
