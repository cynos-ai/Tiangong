...
Issue #11 / Draft PR #12 provide the deterministic target, one-reply correlation, and pre-turn Harness ingress marker without modifying AgentTeams. Issue #13 / Draft PR #14 and the owned receiver provide the lower-layer phase trace. Direct plugin/runtime and long-lived official Gateway probes pass both error/retry and success paths; exact receiver-hardening reproduction then identified the remaining rejection as a test receiver UID/GID versus dropped-capability persistence failure. The receiver identity fix preserves all hardening and validation. Repeated model retries, longer observation windows, or prompt tuning must not be used to manufacture reliability. After the stacked deterministic and CI gates pass, one unchanged bounded attempt may test transport and observability together. Phase 0B remains red until that combined oracle passes.
