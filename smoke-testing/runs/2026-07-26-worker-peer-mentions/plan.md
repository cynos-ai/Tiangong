...
Issue #11 / Draft PR #12 provide the deterministic target, one-reply correlation, and pre-turn Harness ingress marker without modifying AgentTeams. Issue #13 / Draft PR #14 and the owned receiver provide the lower-layer phase trace, but the receiver's remaining unclassified actual-request path must be reproduced or safely isolated before another expensive attempt. Repeated model retries, longer observation windows, or prompt tuning must not be used to manufacture reliability. The next work is receiver/exporter protocol diagnosis at a no-model boundary, not another Matrix/model run. Phase 0B remains red until transport and observability pass together.
