...
**BLOCKED.** Attempts 12–15 prove Admin → Coordinator account visibility and official OpenClaw-to-Tiangong Harness dispatch, but no run has passed the complete strengthened loop. Attempt 13 exposed the receiver root-shape defect, Attempt 14 required receiver counters/inventory, and Attempt 15 proved seven exports were rejected by the receiver's omission of Tiangong's stable `internal_error`. Attempt 15 also had one Engineer message without the required pong marker and no terminal event. Earlier complete loops remain insufficient because they predate bounded terminal correlation or failed terminal quiescence.

Issue #11 / Draft PR #12 provide the deterministic target, one-reply correlation, and pre-turn Harness ingress marker without modifying AgentTeams. Issue #13 / Draft PR #14 and the corrected owned receiver oracle provide the lower-layer phase trace required before another attempt. Repeated model retries, longer observation windows, or prompt tuning must not be used to manufacture reliability. The receiver root shape, counters, unmatched inventory, actual plugin export path, and stable internal-error outcome are now covered by deterministic and same-network no-model probes. No additional real-model retry is justified by channel health or prompt tuning. A future run requires review of the completed observability stack and must preserve every strengthened transport, trace, Leader-silence, and cleanup assertion. Phase 0B remains red until that complete oracle passes.
