...
- **Current pinned-stack status (2026-07-26): BLOCKED.** Embedded `hiclaw apply` in AgentTeams `v1.2.0-beta.1` drops inline `leader.runtime` and `leader.image` at the Team HTTP request boundary, so the actual Leader starts with the stock CoPaw image. Keep the actual-image assertion red until a reviewed public upstream version preserves those fields. See `smoke-testing/runs/2026-07-26-team-runtime-boundary/plan.md`.
