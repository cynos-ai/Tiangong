...
- **Current pinned-stack/runtime status (2026-07-27): BLOCKED.** Explicit public `groupAllowExtra` establishes the peer allowlist. Issue #11 / Draft PR #12 added deterministic reply targeting, bounded one-reply correlation, and a pre-turn Harness marker. Attempts 12–14 proved the exact Admin start was readable by Coordinator and reached its Tiangong Harness (`status=running`), but the Coordinator turn did not complete within the unchanged bounded window. Attempt 13 exposed and fixed the receiver's valid OTLP `parentSpanId:null` root handling; Attempt 14 showed that an empty event-relative selection also needs accepted/rejected counters and a sanitized unmatched inventory. `c051a5f` adds that distinction, and the actual no-model plugin path exports successfully. Earlier complete loops predate bounded terminal correlation or failed terminal quiescence and cannot promote the spike. See `smoke-testing/runs/2026-07-26-worker-peer-mentions/plan.md`.
