...
- The transport observer uses 36 incremental Matrix sync windows with a server timeout of 10 seconds. A correlated `model.start` plus `status=running` when that shorter observer ends proves only that the turn had not completed within the smoke window; it is not evidence that the released 1800-second Harness timeout failed or that the model call would never terminate.
