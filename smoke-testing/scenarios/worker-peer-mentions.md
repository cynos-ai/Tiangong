...
- **Current pinned-stack/runtime status (2026-07-27): BLOCKED.** Explicit public `groupAllowExtra` establishes the peer allowlist. Issue #11 / Draft PR #12 added deterministic reply targeting, bounded one-reply correlation, and a pre-turn Harness marker. Attempts 12–15 proved target-account visibility and Tiangong Harness dispatch but did not pass the complete loop. Attempt 13 fixed valid OTLP `parentSpanId:null` roots; Attempt 14 added receiver counters/inventory; Attempt 15 proved seven requests were rejected because the receiver omitted Tiangong's stable lowercase `internal_error` fallback. `602bbe1` now directly covers and accepts that stable error while retaining the content allowlist. Attempt 15 also contained one Engineer message without the required pong marker and no terminal event. Earlier complete loops predate bounded terminal correlation or failed terminal quiescence and cannot promote the spike. See `smoke-testing/runs/2026-07-26-worker-peer-mentions/plan.md`.
