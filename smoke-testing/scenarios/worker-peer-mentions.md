...
- **Current pinned-stack/runtime status (2026-07-27): BLOCKED.** Explicit public `groupAllowExtra` establishes the peer allowlist. Issue #11 / Draft PR #12 added deterministic reply targeting, bounded one-reply correlation, and a pre-turn Harness marker. Attempt 16 passed the complete bounded start/ping/pong/terminal transport, both Harness markers, terminal quiescence, and Leader silence, but its required event-correlated trace remained empty. Attempts 17–18 prove actual exporter requests reached the receiver and were rejected; the latest rejection was outside all intentional validator categories. `4c075b6` now classifies unknown body/JSON/validation/persistence stages without persisting request content. Failure discipline prohibits another model run until this receiver/exporter protocol path is reproduced at a no-model boundary. No attempt has passed transport and observability together. See `smoke-testing/runs/2026-07-26-worker-peer-mentions/plan.md`.
