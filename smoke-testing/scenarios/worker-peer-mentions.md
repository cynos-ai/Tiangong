...
- **Current pinned-stack/runtime status (2026-07-26): BLOCKED.** Explicit public `groupAllowExtra` establishes the peer allowlist. Issue #11 / Draft PR #12 added deterministic reply targeting, bounded one-reply correlation, and a pre-turn Harness marker. The latest evidence-driven run proved the exact Admin start was readable by Coordinator and reached its Tiangong Harness (`status=running`), but the Coordinator turn did not complete within the unchanged bounded window. Earlier complete loops predate bounded terminal correlation or failed terminal quiescence and cannot promote the spike. See `smoke-testing/runs/2026-07-26-worker-peer-mentions/plan.md`.
