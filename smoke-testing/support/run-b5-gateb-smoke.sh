...
for pair in \
  ...
  runtime="$([[ "${responsibility}" == developer ]] && printf codex-app-server || printf openclaw-built-in)"
  model="$([[ "${responsibility}" == developer ]] && printf '%s' "${CODEX_MODEL}" || printf deepseek-chat)"
