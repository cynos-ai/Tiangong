...
`inject-member-runtime-docker.sh` 只做 MemberConfig 责任、Agent package/version、capability profile、allowedSkills、runtime/model 路由、Coordination endpoint/token 和 Codex canary 参数的原子重建。失败会恢复旧容器；成功后旧容器才清理。运行时选择只有 `openclaw-built-in` 和 `codex-app-server`，两者均使用 `OPENCLAW_AGENT_HARNESS_FALLBACK=none`。
