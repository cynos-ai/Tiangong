...
> 状态：非规范性翻译快照，可能落后于英文主规范。唯一权威合同见
> [`evidence-backed-team-control.md`](evidence-backed-team-control.md)。本文不描述当前实现，也不授权任何交付声明。
>
> 范围：协调、信任与完成、外部效果与授权、组织与行为塑造、质量与环境，以及运行时闭合的目标控制合同翻译。平台特定 adapter 和物理实现拓扑仍为实现工作。
