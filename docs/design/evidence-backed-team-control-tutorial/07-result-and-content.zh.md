...
场景：测试 128 项通过，但一个旧版数据库兼容路径无法在本地复现；没有执行部署。
...
- summary 是否同时写出做到的、没做到的和限制；
- 是否添加了并不存在的 outcome；
- 是否把可变文件路径当 deliverable；
- 是否声称 ToolResult 能证明业务正确；
- 是否擅自写 accept/reject。

## 累积小结：到这里已经学会什么

从 Human 输入到正式交接，完整模型是：

1. 认证消息进入正确 Worker，由 Leader理解，机器权限仍由代码掌握；
2. Work 隔离一整件事务，歧义输入保守建占位并通过 timeline 纠正；
3. WorkSpec 是当前目标完整快照，未知问题等待 Human，后续变化不静默改写 Task；
4. Leader动态创建不可变 Task/TaskSpec，专业工作没有固定角色、阶段、DAG 或通用验证链；
5. 能力来自 AgentTeams、ControlProfile、MemberConfig 和 runtime binding 的当前交集；
6. Skill、消息、RAG 和 Task prose 只提供方法与语义，不授予机器权限；
7. Agent 在长寿命 prepared environment 中使用一等 Bash，控制域与执行域隔离，成员和并行 writer 使用独立 writable root；
8. 私有源码与广泛网络出口分离，Bash 没有外部写凭据；
9. 代码以精确 Git commit 交接，其他内容使用 Adapter 保证的不可变/版本化 ContentRef；
10. Result 是 assignee 对一个 Task 的唯一不可变终态报告，以 taskId 定位；
11. Result 没有结果枚举、独立 ID、验证子类型或 accept/reject；报告内容本身诚实说明限制；
12. ResultGuard 只校验身份、唯一性、Schema、引用稳定性、ToolResult 归属与留存，不判断专业质量；
13. Leader通过后续 Task 和最终 Work closure 表达怎样使用报告；
14. 当前已经有“成员说了什么、交付了什么”，下一步要分清机器工具究竟观察到了什么。

## 自检

1. Git commit、ContentRef 和 Result 各自回答什么问题？
2. 非 Git 内容为什么不能只引用可变路径？
3. Result 为什么不需要 `resultId` 和 outcome？
4. Leader认为 Result 不充分时应该怎样做？
5. ResultGuard 能检查什么，明确不能检查什么？
6. retention 为什么要在 Result 创建前完成？
7. Result 与 Task cancellation 为什么必须原子竞争？
