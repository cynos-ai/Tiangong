...
如果其中两项由同一产品实现也没关系，但必须能说清它在逻辑上承担哪种责任。

## 累积小结：到这里已经学会什么

到第 01 单元为止，完整心智模型是：

1. Human 通过消息通道提出业务请求；
2. 通道提供认证身份、投递位置和稳定消息事件 ID；
3. AgentTeams 管 Worker、平台身份和集成资源是否真实存在；
4. Agent 是持续承担职责的程序角色，模型只是它一次判断所用的组件；
5. Team 有且只有一个 Leader，但成员不被锁死成固定阶段角色；
6. Leader 负责语义判断，代码负责身份、能力和危险动作；
7. 看见消息不等于获得权限，平台身份也不等于 Tiangong 授权；
8. 此时唯一已经发生的业务输入，是一条经过认证来源的 Human 消息。

下一章会给这条消息建立一个稳定业务身份，使它不再只依赖聊天历史。

## 自检

1. Agent 和模型为什么不是同一个概念？
2. Worker 与 Agent 分别回答“在哪里运行”和“谁在承担职责”中的哪一个？
3. AgentTeams 能确认什么，Tiangong 又必须自己确认什么？
4. 为什么通道里的管理员标记不能直接授权生产部署？
5. Leader 拥有语义判断权，为什么仍不能给自己增加凭据？
