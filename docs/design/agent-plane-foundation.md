...
> **待实现设计：** [`reviewer-typed-targets.md`](./reviewer-typed-targets.md) 定义 typed target 与 immutable snapshot 的下一项 clean-cut 合同；在其实现和验证前，本文的显式 `scope.files[]` 与能力限制仍是当前事实。
