...
状态：M7 PostgreSQL 单一产品权威已实现。真实生产验收仍必须在部署系统注入 binding、PG 和 Matrix secret 后执行。
...
- Matrix 负责聊天、历史和消息投递；AgentTeams 共享空间只保存 ContentRef 指向的大对象和报告。
...
产品进程只从 PostgreSQL 读取和写入 Work、Task、Result、Matrix admission、request replay 与 wake outbox。Web 未注入 store 时显示 `postgres-not-configured`，不读取本地 Coordination 文件。Worker 只通过 Control API 访问这些事实。

Matrix admission backlog 负责 Human event 引用的 Room 顺序、lease/retry、Work 关联和纠正；wake outbox 负责内部 Agent 唤醒的 claim/ack 与 Matrix 投递。两者表、actor 和重放合同独立，不合并为通用队列。

当前 runtime 没有 Codex/OpenCodex、Runner、Deployment Adapter、Operation、Approval 或 hash-chain Evidence 组件。后续外部写能力必须和真实 Adapter、typed Operation、精确 Human Approval、恢复与 CloseGuard 同批引入。
